...
export const action = async ({ request }) => {
  ...
  if (!headerShopDomain) {
    return json({ success: false, error: "Missing x-shop-domain header" }, { status: 401, headers: corsHeaders(request) });
  }

  const validSession = await hasValidSessionForShop(headerShopDomain);
  if (!validSession) {
    return json({ success: false, error: "Invalid x-shop-domain: no active session" }, { status: 401, headers: corsHeaders(request) });
  }

  const rateLimit = checkAndIncrementRateLimit(headerShopDomain);
  if (rateLimit.limited) {
    return json({ success: false, error: "Rate limit exceeded", retry_after_seconds: rateLimit.retryAfterSeconds }, { status: 429, headers: corsHeaders(request) });
  }
  ...
  try {
    ...
    return json({ success: false, error: "Invalid JSON body" }, { status: 400, headers: corsHeaders(request) });
  }

  const shopId = normalizeShopDomain(payload?.shop_id || payload?.shopId);
  if (shopId && shopId !== headerShopDomain) {
    return json({ success: false, error: "shop_id must match x-shop-domain header" }, { status: 401, headers: corsHeaders(request) });
  }
  ...
  const productImage = nonEmptyString(productImageRaw) ? productImageRaw.trim() : null;
  ...
  if (!userImage || !productImage) {
    return json({ success: false, error: "Missing required params: garmentImageUrl and personImageBase64 or personImageUrl" }, { status: 400, headers: corsHeaders(request) });
  }

  if (headerShopDomain && productId) {
    const allowedResult = await isProductAllowed(String(headerShopDomain), String(productId));
    if (!allowedResult.allowed) {
      return json({ success: false, error: "LIMIT_REACHED", reason: "LIMIT_REACHED", plan_name: allowedResult.planName, max_products_allowed: allowedResult.maxProductsAllowed, active_products_count: allowedResult.activeProductsCount }, { status: 403, headers: corsHeaders(request) });
    }
  }
  ...
  if (!tryOnQuota.allowed) {
    return json({ success: false, error: "TRYON_QUOTA_EXCEEDED", reason: "TRYON_QUOTA_EXCEEDED", plan_name: tryOnQuota.planKey, max_tryons_per_month: tryOnQuota.maxTryOnsPerMonth, current_tryons_count: tryOnQuota.currentTryOnsCount }, { status: 403, headers: corsHeaders(request) });
  }
  ...
  if (!result.success) {
    ...
    return json({ success: false, error: result.error || "TRYON_FAILED" }, { status: 500, headers: corsHeaders(request) });
  }

  await logTryOn(headerShopDomain, typeof productId === "string" ? productId : null, "success", result.imageUrl, null);

  return json({ success: true, imageUrl: result.imageUrl, mock: result.mock || false }, { headers: corsHeaders(request) });
};
